...
| `mealie_mealplan.py` | Lädt den Essenplan der aktuellen Woche (Montag-Sonntag) aus Mealie (API v1) und formatiert ihn nach Tag gruppiert als Markdown. | `MEALIE_BASE_URL`, `MEALIE_API_TOKEN` |
| `mealie_recipe_today.py` | Lädt das erste für heute im Essenplan hinterlegte Rezept aus Mealie (API v1): Zutaten/Zubereitung als Markdown (`generate()`) und das Rezeptfoto als Bild-Anhang (`generate_image()`). | `MEALIE_BASE_URL`, `MEALIE_API_TOKEN` |
...
| `news_important.py` | Authentifiziert sich per OAuth2-Client-Credentials-Flow (Authentik-Service-Account) gegen Authentik und lädt die obersten, bereits nach Score priorisierten Einträge aus dem News-Aggregator (`GET /api/v1/news`). `NEWS_CLIENT_ID` muss die Client-ID des OAuth2-Providers der News-App selbst sein (Audience-Prüfung). | `NEWS_BASE_URL`, `NEWS_AUTHENTIK_TOKEN_URL`, `NEWS_CLIENT_ID`, `NEWS_AUTHENTIK_USERNAME`, `NEWS_AUTHENTIK_PASSWORD` |
