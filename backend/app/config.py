...
class Settings(BaseSettings):
    ...
    news_authentik_token_url: str | None = None
    # Client-ID MUSS die des OAuth2-Providers sein, den die News-App selbst fuer
    # ihren eigenen Login verwendet (deren AUTHENTIK_CLIENT_ID) - der News-Backend
    # prueft beim Validieren des Access-Tokens "audience == eigene client_id"
    # (siehe app/auth/authentik.py::_verify_jwt im News-Projekt). Ein separat in
    # Authentik angelegter Provider mit eigener Client-ID wuerde also mit
    # "Invalid audience" abgelehnt.
    news_client_id: str | None = None
    # Benutzername/App-Passwort (Token) eines Authentik-Service-Accounts
    # (Directory -> Tokens and App passwords bzw. "Create Service account") -
    # Authentiks client_credentials-Flow authentifiziert ueber diese beiden
    # Felder, nicht ueber ein klassisches client_secret.
    news_authentik_username: str | None = None
    news_authentik_password: str | None = None
...
WEB_SETTINGS_FIELDS: dict[str, SettingFieldSpec] = {
    spec.key: spec
    for spec in (
        SettingFieldSpec("mealie_base_url", "Mealie", "Basis-URL"),
        SettingFieldSpec("mealie_api_token", "Mealie", "API-Token", secret=True),
        SettingFieldSpec("mealie_shopping_list_id", "Mealie", "Einkaufslisten-ID (optional)"),
        ...
        SettingFieldSpec("news_base_url", "News-Aggregator", "Basis-URL"),
        SettingFieldSpec("news_authentik_token_url", "News-Aggregator", "Authentik Token-URL"),
        SettingFieldSpec("news_client_id", "News-Aggregator", "Client-ID (der News-App)"),
        SettingFieldSpec("news_authentik_username", "News-Aggregator", "Service-Account-Benutzername"),
        SettingFieldSpec("news_authentik_password", "News-Aggregator", "Service-Account-Token", secret=True),
    )
}
