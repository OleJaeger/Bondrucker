...
def test_mealie_mealplan_without_config_raises(settings_env):
    with pytest.raises(PresetScriptError):
        mealie_mealplan.generate()


def test_mealie_mealplan_formats_week(settings_env, monkeypatch):
    monkeypatch.setenv("MEALIE_BASE_URL", "https://mealie.example.com")
    monkeypatch.setenv("MEALIE_API_TOKEN", "token123")
    get_settings.cache_clear()
    ...
    def _fake_get(url, headers=None, params=None, timeout=None):
        assert url.endswith("/api/households/mealplans")
        assert params["start_date"] == monday.isoformat()
        assert params["end_date"] == (monday + timedelta(days=6)).isoformat()
        return _FakeResponse(
            json_data={
                "items": [
                    {"date": monday.isoformat(), "entryType": "dinner", "recipe": {"name": "Chili"}},
                    {"date": monday.isoformat(), "entryType": "breakfast", "title": "Haferbrei"},
                    {"date": wednesday.isoformat(), "entryType": "lunch", "recipe": {"name": "Salat"}},
                ]
            }
        )

    monkeypatch.setattr(mealie_mealplan.httpx, "get", _fake_get)

    monday_name = mealie_mealplan._WEEKDAYS[monday.weekday()]
    wednesday_name = mealie_mealplan._WEEKDAYS[wednesday.weekday()]

    assert mealie_mealplan.generate() == (
        f"## {monday_name}, {monday.strftime('%d.%m.')}\n"
        "- Haferbrei (Fruehstueck)\n"
        "- Chili (Abendessen)\n"
        "\n"
        f"## {wednesday_name}, {wednesday.strftime('%d.%m.')}\n"
        "- Salat (Mittagessen)"
    )


def test_mealie_mealplan_no_entries(settings_env, monkeypatch):
    monkeypatch.setenv("MEALIE_BASE_URL", "https://mealie.example.com")
    monkeypatch.setenv("MEALIE_API_TOKEN", "token123")
    get_settings.cache_clear()

    monkeypatch.setattr(mealie_mealplan.httpx, "get", lambda *a, **kw: _FakeResponse(json_data={"items": []}))

    assert mealie_mealplan.generate() == "Fuer diese Woche ist kein Essenplan hinterlegt."


def test_mealie_mealplan_http_error(settings_env, monkeypatch):
    monkeypatch.setenv("MEALIE_BASE_URL", "https://mealie.example.com")
    monkeypatch.setenv("MEALIE_API_TOKEN", "token123")
    get_settings.cache_clear()
...
def _set_mealie_env(monkeypatch) -> None:
    monkeypatch.setenv("MEALIE_BASE_URL", "https://mealie.example.com")
    monkeypatch.setenv("MEALIE_API_TOKEN", "token123")
    get_settings.cache_clear()


def test_mealie_recipe_today_without_config_raises(settings_env):
    with pytest.raises(PresetScriptError):
        mealie_recipe_today.generate()
    with pytest.raises(PresetScriptError):
        mealie_recipe_today.generate_image()


def _mealplan_response(today, recipes):
    return _FakeResponse(
        json_data={
            "items": [
                {"date": today.isoformat(), "entryType": entry_type, "recipe": {"slug": slug}}
                for entry_type, slug in recipes
            ]
        }
    )


def test_mealie_recipe_today_formats_recipe(settings_env, monkeypatch):
    _set_mealie_env(monkeypatch)
    today = date.today()

    def _fake_get(url, headers=None, params=None, timeout=None):
        if url.endswith("/api/households/mealplans"):
            assert params["start_date"] == today.isoformat()
            assert params["end_date"] == today.isoformat()
            return _mealplan_response(today, [("dinner", "chili"), ("breakfast", "haferbrei")])
        assert url.endswith("/api/recipes/haferbrei")
        return _FakeResponse(
            json_data={
                "name": "Haferbrei",
                ...
                "recipeIngredient": [
                    {"display": "100 g Haferflocken"},
                    {"food": {"name": "Milch"}},
                    {"note": "Prise Salz"},
                ],
                "recipeInstructions": [
                    {"text": "Milch erhitzen."},
                    {"text": "Haferflocken einruehren."},
                    {"text": "  "},
                ],
            }
        )

    monkeypatch.setattr(mealie_recipe_today.httpx, "get", _fake_get)

    assert mealie_recipe_today.generate() == (
        "## Haferbrei\n"
        ...
        "### Zubereitung\n"
        "1. Milch erhitzen.\n"
        "2. Haferflocken einruehren."
    )


def test_mealie_recipe_today_no_recipe_planned(settings_env, monkeypatch):
    _set_mealie_env(monkeypatch)

    monkeypatch.setattr(
        mealie_recipe_today.httpx, "get", lambda *a, **kw: _FakeResponse(json_data={"items": []})
    )

    assert mealie_recipe_today.generate() == "Fuer heute ist kein Rezept im Essenplan hinterlegt."


def test_mealie_recipe_today_generate_image_no_recipe_planned_raises(settings_env, monkeypatch):
    _set_mealie_env(monkeypatch)
...
class _FakeImageResponse(_FakeResponse):
...
def test_mealie_recipe_today_generate_image_returns_bytes(settings_env, monkeypatch):
    _set_mealie_env(monkeypatch)
    today = date.today()

    def _fake_get(url, headers=None, params=None, timeout=None):
        if url.endswith("/api/households/mealplans"):
            return _mealplan_response(today, [("dinner", "chili")])
        if url.endswith("/api/recipes/chili"):
            return _FakeResponse(json_data={"id": "recipe-uuid-1", "image": "abc123"})
        assert url.endswith("/api/media/recipes/recipe-uuid-1/images/original.webp")
        return _FakeImageResponse(b"IMAGEDATA")

    monkeypatch.setattr(mealie_recipe_today.httpx, "get", _fake_get)

    assert mealie_recipe_today.generate_image() == b"IMAGEDATA"


def test_mealie_recipe_today_generate_image_without_image_raises(settings_env, monkeypatch):
    _set_mealie_env(monkeypatch)
    today = date.today()
    ...
    def _fake_get(url, headers=None, params=None, timeout=None):
        if url.endswith("/api/households/mealplans"):
            return _mealplan_response(today, [("dinner", "chili")])
        assert url.endswith("/api/recipes/chili")
        return _FakeResponse(json_data={"id": "recipe-uuid-1", "image": None})
...
def test_mealie_recipe_today_http_error_raises(settings_env, monkeypatch):
    _set_mealie_env(monkeypatch)
...
def _set_news_env(monkeypatch) -> None:
    monkeypatch.setenv("NEWS_BASE_URL", "https://news.example.com")
    monkeypatch.setenv("NEWS_AUTHENTIK_TOKEN_URL", "https://auth.example.com/application/o/token/")
    monkeypatch.setenv("NEWS_CLIENT_ID", "client-id")
    monkeypatch.setenv("NEWS_AUTHENTIK_USERNAME", "ak-bondrucker")
    monkeypatch.setenv("NEWS_AUTHENTIK_PASSWORD", "app-password-token")
    get_settings.cache_clear()


def _reset_news_token_cache(monkeypatch) -> None:
    monkeypatch.setattr(news_important, "_cached_token", None)
    monkeypatch.setattr(news_important, "_cached_token_expires_at", 0.0)


def test_news_important_without_config_raises(settings_env, monkeypatch):
    _reset_news_token_cache(monkeypatch)
    with pytest.raises(PresetScriptError):
        news_important.generate()


def test_news_important_formats_news_list(settings_env, monkeypatch):
    _set_news_env(monkeypatch)
    _reset_news_token_cache(monkeypatch)

    def _fake_post(url, data=None, timeout=None):
        assert url == "https://auth.example.com/application/o/token/"
        assert data["grant_type"] == "client_credentials"
        assert data["client_id"] == "client-id"
        assert data["username"] == "ak-bondrucker"
        assert data["password"] == "app-password-token"
        return _FakeResponse(json_data={"access_token": "the-token", "expires_in": 3600})

    def _fake_get(url, headers=None, params=None, timeout=None):
        assert url == "https://news.example.com/api/v1/news"
        assert headers == {"Authorization": "Bearer the-token"}
        return _FakeResponse(
            json_data=[
                {
                    "canonical_title": "Wichtige Meldung",
                    "canonical_summary": "Kurzer Text dazu.",
                    "category": "Technik",
                },
                {"canonical_title": "Zweite Meldung", "canonical_summary": None, "category": None},
            ]
        )

    monkeypatch.setattr(news_important.httpx, "post", _fake_post)
    monkeypatch.setattr(news_important.httpx, "get", _fake_get)

    content = news_important.generate()
    assert "Wichtige Meldung" in content
    assert "Technik" in content
    assert "Kurzer Text dazu." in content
    assert "Zweite Meldung" in content


def test_news_important_empty_list_returns_placeholder(settings_env, monkeypatch):
    _set_news_env(monkeypatch)
    _reset_news_token_cache(monkeypatch)

    monkeypatch.setattr(
        news_important.httpx,
        "post",
        lambda *a, **k: _FakeResponse(json_data={"access_token": "tok", "expires_in": 3600}),
    )
    monkeypatch.setattr(news_important.httpx, "get", lambda *a, **k: _FakeResponse(json_data=[]))

    assert news_important.generate() == "Aktuell liegen keine wichtigen Nachrichten vor."


def test_news_important_token_error_raises_preset_script_error(settings_env, monkeypatch):
    _set_news_env(monkeypatch)
    _reset_news_token_cache(monkeypatch)
...
def test_news_important_news_http_error_raises_preset_script_error(settings_env, monkeypatch):
    _set_news_env(monkeypatch)
    _reset_news_token_cache(monkeypatch)
...
def test_news_important_caches_token_across_calls(settings_env, monkeypatch):
    _set_news_env(monkeypatch)
    _reset_news_token_cache(monkeypatch)
